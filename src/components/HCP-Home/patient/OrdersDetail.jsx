...
const INITIAL_ORDERS = [
  { id: 1, datetime: '16/01/2025 - 10:00 AM', orderType: 'Labs',    order: 'Complete Blood Count (CBC)', priority: 'Routine', status: 'Submitted', comment: '' },
  { id: 2, datetime: '17/01/2025 - 10:00 AM', orderType: 'Labs',    order: 'Liquid Panel',               priority: 'STAT',    status: 'Submitted', comment: '' },
  { id: 3, datetime: '18/01/2025 - 10:00 AM', orderType: 'Imaging', order: 'X-Ray',                      priority: 'Timed',   status: 'Draft',     comment: '' },
  { id: 4, datetime: '19/01/2025 - 10:00 AM', orderType: 'Imaging', order: 'MRI',                        priority: 'Timed',   status: 'Submitted', comment: '' },
  { id: 5, datetime: '20/01/2025 - 10:00 AM', orderType: 'Labs',    order: 'Basic Metabolic Panel',      priority: 'Routine', status: 'Submitted', comment: '' },
];

const ROWS_PER_PAGE = 8;

const STATUS_STYLE = {
  Submitted: { bg: 'rgba(18,183,106,0.1)',    border: 'rgba(18,183,106,0.1)',    color: '#12B76A'                   },
  Draft:     { bg: 'rgba(222,226,230,0.45)',  border: 'rgba(222,226,230,0.45)',  color: 'rgba(103,120,131,0.72)'    },
};

/* ── OrdersDetail ────────────────────────────────── */
const OrdersDetail = ({ patient, chart }) => {
  ...
  const handleSave = (data) => {
    if (data.id) {
      setOrders(prev => prev.map(r => r.id === data.id ? data : r));
    } else {
      setOrders(prev => [...prev, { ...data, id: Date.now() }]);
    }
    setShowPanel(false);
    setViewRow(null);
  };
  ...
  const handleDelete = () => {
    setOrders(prev => prev.filter(r => r.id !== deleteTarget.id));
  ...
  return (
    <div className="flex flex-col gap-4" style={{ width: '100%' }}>
      ...
      <div
        ...
        style={{ background: '#FFFFFF', border: '1px solid #E1EBF5', borderRadius: '12px', padding: '16px' }}
      >
      ...
      <div
        ...
        style={{ background: '#FFFFFF', border: '1px solid #E0E0E0', borderRadius: '12px', padding: '24px' }}
      >
        {/* Header */}
        <div className="flex items-center justify-between" style={{ height: '48px' }}>
          <span className="text-[20px] font-medium" style={{ color: '#222126' }}>Orders</span>
          <button
          ...
          >
            ...
            <span className="text-[16px] font-semibold text-white whitespace-nowrap">Add New Order</span>
          </button>
        </div>

        {/* Search bar */}
        <div className="flex items-center gap-6" style={{ height: '48px' }}>
          <div
            className="flex items-center gap-2 px-6"
            style={{ width: '380px', height: '48px', border: '1px solid rgba(158,158,158,0.4)', borderRadius: '26px' }}
          >
              ...
              >
                ...
                <div style={{ flex: 1, padding: '16px 24px', overflow: 'hidden' }}>
                  <span
                    ...
                    style={{ color: '#1B1B28', letterSpacing: '0.25px' }}
                  >
                    {row.orderType}
                  </span>
                </div>
        ...
        <div className="flex items-center justify-between" style={{ height: '24px' }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => setPage(1)}
              disabled={safePage === 1}
              className="w-6 h-6 flex items-center justify-center rounded cursor-pointer disabled:opacity-40"
      ...
      {deleteTarget && (
        <DeleteConfirmModal
          ...
          description="Deleting this Order will permanently remove it from your Patient Orders."
